function [ output_args ] = aggregate_gist_feature( proj_name, feature_ext, kf_dir_name, szPat, start_seg, end_seg )
...
%% kf_dir_name: name of keyframe folder, e.g. keyframe-60 for segment length of 60s   
    ...
    parfor ii = start_seg:end_seg,
		...
		for jj = start_kf:end_kf,
			...
			feat_file = sprintf('%s/%s/%s.mat', output_kf_feat_dir, info.video, img_name(1:end-4));
			if ~exist(feat_file, 'file'),
				warning('File not found! [%s] ', feat_file);
				continue;
			end
			
			descrs = load(feat_file, 'gist'); 	% 1x512
			descrs = descrs.gist'; 				% 512x1
